...
function logProof(label, user, nonceHex) {
  const challenge_bytes = hexToBytes(nonceHex);
  const proof = generate_proof(user.secret_key, user.pub_key, challenge_bytes);
...
async function main() {
  console.log('Testing Zynk proof generation with changing nonces...\n');

  const user = createTestUser('alice@example.com');

  // First \"login\" with nonce1
  const nonce1 = generateNonceHex();
  const proof1 = logProof('Login 1', user, nonce1);

  // Second \"login\" with nonce2
  const nonce2 = generateNonceHex();
  const proof2 = logProof('Login 2', user, nonce2);

  // Third \"login\" reusing nonce1 to show determinism
  const proof3 = logProof('Login 3 (reuse nonce1)', user, nonce1);
  ...
  console.log('\nSame key, same nonce -> proofs should be IDENTICAL:');
